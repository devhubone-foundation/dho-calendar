...
2. Wait for the first build + `preDeployCommand` (`prisma:migrate:deploy`) + deploy to finish.
...
5. Open **Shell** on the service and run the seed once:
   ```bash
   pnpm --filter @dho/database prisma:seed
   ```
6. Visit the public URL — the public calendar should render; log in with the seeded admin credentials (see the seed's console output for the current run's demo dates) and confirm the forced password-change flow.
